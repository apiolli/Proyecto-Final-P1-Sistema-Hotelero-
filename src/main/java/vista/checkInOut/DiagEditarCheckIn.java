...
public class DiagEditarCheckIn extends javax.swing.JDialog {
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(DiagEditarCheckIn.class.getName());
    
    private controlador.ControladorChecks controlador; 
    private int idReservaActual;
    private int idHabitacionActual;
    
    public DiagEditarCheckIn(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    public void cargarDatos(int idReserva, String documento, String nombreCompleto, java.util.Date fEntrada, java.util.Date fSalida, int numPersonas, String numHabitacion, double dineroAbonado) {
        this.idReservaActual = idReserva;
        txtID.setText(String.valueOf(idReserva));
        

        txtDocumento.setText(documento);
        txtDocumento.setEditable(false);
        btnBuscarDoc.setVisible(false);
        ...
        fechaEntrada.setDate(fEntrada);
        fechaEntrada.setEnabled(true); 
        fechaSalida.setDate(fSalida);
        spNoPersonas.setValue(numPersonas);
        
        cmbHabDisponibles.removeAllItems();
        cmbHabDisponibles.addItem(numHabitacion);
        cmbHabDisponibles.setSelectedItem(numHabitacion);
        this.idHabitacionActual = Integer.parseInt(numHabitacion);
        
        txtDineroAbonado.setText(String.valueOf(dineroAbonado));
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Gestionar Check-In");
    ...
    private void btnDIagActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDIagActionPerformed
        try {
            java.awt.Component boton = (java.awt.Component) evt.getSource();
            boton.setEnabled(false);

            if (fechaEntrada.getDate() == null || fechaSalida.getDate() == null) {
                JOptionPane.showMessageDialog(this, "Seleccione fechas válidas.", "Validación", JOptionPane.WARNING_MESSAGE);
                boton.setEnabled(true); return;
            }
            if (cmbHabDisponibles.getSelectedItem() == null) {
                JOptionPane.showMessageDialog(this, "Seleccione una habitación.", "Validación", JOptionPane.WARNING_MESSAGE);
                boton.setEnabled(true); return;
            }

            boolean exito = controlador.editarCheckIn(idReservaActual, idHabitacionActual);
            ...
            ((java.awt.Component) evt.getSource()).setEnabled(true);
        }
    ...
    public void setControlador(controlador.ControladorChecks controlador) {
