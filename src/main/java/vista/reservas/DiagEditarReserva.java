...
public class DiagEditarReserva extends javax.swing.JDialog {
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(DiagEditarReserva.class.getName());
    
    private ControladorReserva controlador;
    private int idReservaActual;
    
    public DiagEditarReserva(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    // Mostramos el documento y bloqueamos la edición de identidad
    txtDocumento.setText(documento);
    txtDocumento.setEditable(false);
    
    // Dividimos el nombre para repartirlo en los campos
    ...
    // El resto de los datos (estos sí se podrán editar)
    ...
    private void btnDIagActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDIagActionPerformed
        // 1. Detectamos el botón que fue presionado y lo desactivamos
        java.awt.Component boton = (java.awt.Component) evt.getSource();
        boton.setEnabled(false);

        // 2. Validaciones (Fechas y Habitación)
        ...
        // 3. LLAMAMOS AL CONTROLADOR PARA EDITAR EN LA BASE DE DATOS
        boolean exito = controlador.editarReserva(idReservaActual);
        
        if (exito) {
            this.dispose(); // Si funcionó, cerramos la ventana
        } else {
            boton.setEnabled(true); // Si falló algo (ej. excedió capacidad), volvemos a encender el botón
        }
